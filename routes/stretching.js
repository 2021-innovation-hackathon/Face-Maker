...
router.get("/count", function (req, res) {
  db.query(
    ...
    function (err2, re) {
      ...
      db.query(
        `UPDATE stretching set count=${re[0].count}+1 WHERE stretching_id=1`,
        function (err, result) {
          if (err) throw err;
...
router.get("/average/:pageId", function (req, res) {
  ...
  db.query(
    `UPDATE stretching set averagecount=averagecount+1, sumscore= sumscore+${req.params.pageId} WHERE stretching_id=1`,
    function (err, result) {
      if (err) throw err;
      db.query(
        `UPDATE stretching set average=sumscore/averagecount where stretching_id=1`,
